...
synthetic fixture regenerated byte-for-byte; its retained original stays unchanged. All 18 combined local gates passed on source
`c57ffb96bbee64cf4af541a04f6c730cbb215f8a`: seven repository gates, four dedicated integrations
(Decision Case 16, Shared Analysis 2, Transport 12, Episode 2; zero skips), pack, registry,
Ruff, mypy, 75 Python tests, frozen assessment-packet equality and retained pilot receiving.
The [source-bound receipt](decision-episode-repair-evidence.json) retains commands, log hashes,
source manifest identity, preservation comparisons and the failed first attempt. That attempt
caught formatting, a default-runner timeout and dirty/checksum rejection during the formatting
repair. The successful rerun used a clean committed tree and dedicated integration entrypoints.
Hosted checks are evaluated separately against the final PR head; local integration passes do not
claim those external backends ran in hosted CI.
