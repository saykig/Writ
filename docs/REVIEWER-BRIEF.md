# Reviewer brief: the Gap Matrix, through Covenant

For **Sara** (or any Gap Matrix author). This asks you to check one thing: does
Covenant represent your methodology faithfully, and does its analyzer point at
the right places? It takes about fifteen minutes and needs no code.

## What Covenant did with your Gap Matrix
...
- See it: **covenant-dsl.vercel.app/gap-matrix**
- The methodology: `examples/ai-governance-gap-matrix.covenant`
...
1. **The indices.** Covenant computes **public authority = 50** (all five
...
4. **The public gate — a known divergence, please rule on it.** Your engine gates
   ...
   Covenant currently gates only on *pending*, so it would publish public
   authority = 50 where you would hold it null pending review. **Which gate is
   right?** If yours, we should tighten Covenant to your assessment-status gate.

5. **The encoding.** Covenant treats each analyst assessment as a reviewed
...
- It **does** tell us whether Covenant represents your methodology faithfully and
...
- It **does not** validate the scores themselves. Those are your judgment;
  Covenant only recomputes and audits them. A "yes" here means "the tool is
