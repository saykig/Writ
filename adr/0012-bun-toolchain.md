...
- Bun executes TypeScript source directly, so internal packages resolve `@covenant/*` to `./src/index.ts` via `exports`; `build`/`typecheck` are `tsc --noEmit` (type safety without a dist artifact to manage).
