#!/usr/bin/env bash
# Covenant end-to-end demonstration.
...
# covers @covenant/{language,provenance,evaluator,domain,analyzer,benchmark};
# repo files are referenced through $ROOT so cwd does not matter.
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"
export ROOT
CLI() { bun "$ROOT/packages/cli/bin/covenant.ts" "$@"; }
bench() { (cd "$ROOT/packages/benchmark" && bun -e "$1"); }
rule() { printf '\n\033[1m%s\033[0m\n' "════ $1 ════"; }

rule "1. LANGUAGE — compile Covenant source to the canonical IR (golden hash)"
bench '
...
const r = compileSource(readFileSync(root + "/examples/2025-ai-sme-literal.covenant","utf8"), { fileName: "demo" });
...
CLI analyze "$ROOT/examples/2025-ai-sme-literal.covenant" 2>&1 | grep -E "COV-SCORE" | sed 's/^/    /' || true
echo "  resolved reading:"
CLI analyze "$ROOT/examples/2025-ai-sme-resolved.covenant" 2>&1 | tail -1 | sed 's/^/    /'
...
bench '
import { runBenchmark } from "@covenant/benchmark";
import { writeFileSync } from "node:fs";
const run = await runBenchmark();
const rec = run.receipts.get("canada");
writeFileSync("/tmp/covenant-demo-receipt.json", JSON.stringify(rec, null, 2));
const t = JSON.parse(JSON.stringify(rec)); t.result = "-1";
writeFileSync("/tmp/covenant-demo-receipt-tampered.json", JSON.stringify(t, null, 2));
console.log("  canada receipt:", rec.result, "/", rec.result_status);
'
echo "  verify authentic:"; CLI receipt verify /tmp/covenant-demo-receipt.json 2>&1 | sed 's/^/    /'
echo "  verify tampered (result field flipped +1 -> -1):"; (CLI receipt verify /tmp/covenant-demo-receipt-tampered.json 2>&1 || true) | sed 's/^/    /'
