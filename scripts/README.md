...
- `validate_pack.py` and `validate_pack.sh` check schemas, examples, task metadata, and semantic
...
- `emit_g20_rio.py`, `migrate_eu_us_corpora.py`, and `publish_corpus.ts` are deterministic migration
  ...
  corpus locations.
- `generate_source_registry.py` builds the compatibility projection in `data/` from reviewed
  configuration.
...
- `replicate.ts` and `demo.sh` reproduce the historical G7 evaluator benchmark from frozen inputs.
