...
function parseCanonicalRecord(bytes: Uint8Array): CertificateTransportRecord {
  ...
  try {
    ...
    throw new SharedAnalysisError(
      "SHARED_ANALYSIS_TRANSPORT_BINDING_INVALID",
      "Certificate transport record is not valid UTF-8 JSON.",
    );
  }
...
function requireSubstantiveTransportRequest(requestBytes: Uint8Array): void {
  let request: Record<string, unknown>;
  try {
    request = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(requestBytes)) as Record<
      string,
      unknown
    >;
  } catch {
    throw new SharedAnalysisError(
      "SHARED_ANALYSIS_TRANSPORT_BINDING_INVALID",
      "Certificate transport request is not valid JSON.",
    );
  }
  ...
  if (
    ...
    throw new SharedAnalysisError(
      "SHARED_ANALYSIS_TRANSPORT_BINDING_INVALID",
      "Certificate transport request does not expose source and target declarations.",
    );
  }
  const sourceRecord = source as Record<string, unknown>;
  const targetRecord = target as Record<string, unknown>;
  const changed =
    exactJsonKey(sourceRecord.subject) !== exactJsonKey(targetRecord.subject) ||
    exactJsonKey(sourceRecord.policy) !== exactJsonKey(targetRecord.policy);
  if (!changed) {
    throw new SharedAnalysisError(
      "SHARED_ANALYSIS_TRANSPORT_BINDING_INVALID",
      "This integration requires a substantive target model or policy revision, not a no-op transport.",
    );
  }
}
...
function supportContext(
  ...
  return { impact, scoped, basis, assessment };
}

function normalizedModelBinding(
  declaration: CertificateTransportModelBinding,
): CertificateTransportModelBinding {
  if (declaration.rationale.trim().length === 0 || declaration.changed_request_fields.length === 0) {
    throw new SharedAnalysisError(
      "SHARED_ANALYSIS_TRANSPORT_BINDING_INVALID",
...
function validateLoadedRecord(value: CertificateTransportRecord): LoadedCertificateTransportRecord {
  ...
  requireSubstantiveTransportRequest(requestBytes);
  ...
  if (
    ...
    throw new SharedAnalysisError(
      ...
      "Stored producer-time transport check is not a fully checked target-and-transport warrant.",
    );
  }
...
export function createCertificateTransportRecord(
  ...
  const normalizedBinding = normalizedModelBinding(modelBinding);
  requireSubstantiveTransportRequest(requestBytes);
