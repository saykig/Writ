...
[`proof.md`](./proof.md) and the internal compatibility script at
`internal/verification/fixtures/compatibility/g7-ai-sme/demo.sh`.
...
The evidence is frozen in-repo and needs no network. The internal compatibility script reproduces
the AI-for-SMEs figures; `bun run replicate` re-derives every
...
2. ✅ **Frozen re-derivation of the evidence.** Done — `bun run replicate`
