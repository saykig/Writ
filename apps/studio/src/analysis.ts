 ...
 * `packages/cli/src/analysis.ts` `analyzeCommitment`.
 *
 * The studio server runs this on every keystroke, so it uses the deterministic,
 * Z3-free bounded-enumeration pass (`analyzeScoreProgramByEnumeration`) rather
 * than spinning up the SMT solver. The two passes agree on the seeded programs
 * by construction. Only `@covenant/analyzer` and `@covenant/domain` are imported
 * (both direct dependencies of `@covenant/studio`); the CLI package is not a
 * dependency and is not touched.
 */
...
/** Expand an assertion domain (numeric range or explicit set) into concrete values. */
...
/** Finite domains for a commitment, unioned across all of its assertion domains. */
