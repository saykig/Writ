...
for i in 1:N-1
    ...
    remembered_tests = ["T$j" for j in 1:i]
    add_node!(diagram, DecisionNode("D$i", remembered_tests, ["treat", "pass"]))
...
write_result("memory-leak-result.json", diagram, strategy, expected_utility, status; source = "full-memory-donor-variant")
println("Full-memory donor expected utility: $(expected_utility)")
